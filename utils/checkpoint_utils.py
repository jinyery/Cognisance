...
class Checkpoint:
    ...
    def save(
        self, model, classifier, epoch, logger, add_dict={}, acc=None, cat_clf=None
    ):
        # only save at certain steps or the last epoch
        if epoch % self.config["checkpoint_opt"]["checkpoint_step"] != 0 and epoch < (
            self.config["training_opt"]["num_epochs"] - 1
        ):
            return
        ...
        model_name = (
            "epoch_{}_".format(epoch) + self.config["checkpoint_opt"]["checkpoint_name"]
        )
        ...
        if cat_clf is not None:
            clf_dir = os.path.join(
                self.config["output_dir"], "clf", "epoch_" + str(epoch)
            )
            if not os.path.exists(clf_dir):
                os.makedirs(clf_dir)
            for key, value in cat_clf.items():
                value.save(path=os.path.join(clf_dir, str(key) + ".clf"))
        ...
        logger.info(
            "Best model is at epoch {} with accuracy {:9.3f}".format(
                self.best_epoch, self.best_performance
            )
        )

    def save_best_model_path(self, logger):
        logger.info(
            "Best model is at epoch {} with accuracy {:9.3f} (Path: {})".format(
                self.best_epoch, self.best_performance, self.best_model_path
            )
        )
        with open(
            os.path.join(self.config["output_dir"], "best_checkpoint"), "w+"
        ) as f:
            f.write(
    ...
    def load(self, model, classifier, path, logger):
        ...
        logger.info(
            "Loading checkpoint pretrained with epoch {}.".format(checkpoint["epoch"])
        )
    ...
    def load_module(self, module, module_state, logger):
        ...
        for key, _ in x.items():
            if key in module_state:
                ...
                logger.info(
                    "Load {:>50} from checkpoint (rematch with module.).".format(key)
                )
            else:
