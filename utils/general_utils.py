...
def update(config, algo_config, args):
    ...
    if args.train_type:
        ...
        config["num_sampler"] = (
            algo_info["num_sampler"] if "num_sampler" in algo_info else 1
        )
        config["batch_split"] = (
            algo_info["batch_split"] if "batch_split" in algo_info else False
        )
        ...
        config["algorithm_opt"]["mix_up"] = (
            algo_info["mix_up"] if "mix_up" in algo_info else False
        )
        config["dataset"]["rand_aug"] = (
            algo_info["rand_aug"] if "rand_aug" in algo_info else False
        )
