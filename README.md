...
```
Run the following command to train a center_dual_cos model on Train-GLT of MSCOCO-GLT:
```
...
```
Run the following command to test a center_dual_cos model on Train-GLT of MSCOCO-GLT:
```
python main.py --cfg config/COCO_LT.yaml  --output_dir checkpoints/coco_glt/test/center_dual_cos --require_eval --train_type center_dual_cos --phase test --load_dir checkpoints/coco_glt/train/center_dual_cos/YOUR_CHECKPOINT.pth
```
