...
INVALID_ATTRIBUTE_DATASET = ('MSCOCO-LT', 'MSCOCO-BL', 'Places-GLT')
...
class test_tde():
    ...
    def run_test(self):
        ...
        with torch.no_grad():
            ...
            # load embed
            self.embed = self.add_ckpt['embed'].cuda()
            ...
            for _, (inputs, labels, freq_labels, attributes, indexes) in enumerate(self.loader):
                ...
                add_inputs = {}
                add_inputs['embed'] = self.embed.view(1, -1).repeat(inputs.shape[0], 1)
    ...
    def run_val(self, epoch, embed=None):
