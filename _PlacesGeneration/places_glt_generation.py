...
# class-wise balance
def sampling_val_set(size_of_val_set, cat_inst: dict, used_inst: list, activated_cat:list=None):
    ...
    for cat, insts in cat_inst.items():
        if activated_cat is not None and cat not in activated_cat:
            continue
        insts = list(set(insts) - set(used_inst))
        if activated_cat is None:
            num_of_sample = int(size_of_val_set / len(cat_inst))
        else:
            num_of_sample = int(size_of_val_set / len(activated_cat))
        if num_of_sample > len(insts):
            print(
                f"Warning: Request {num_of_sample}, only {len(insts)} for category-{cat}!"
            )
...
# long-tail
def sampling_test_set(size_of_test_set, cat_inst: dict, used_inst: list, activated_cat:list=None):
...
# class-wise balance
def sampling_test_set_cbl(size_of_test_set_cbl, cat_inst: dict, used_inst: list, activated_cat:list=None):
    ...
    for cat, insts in cat_inst.items():
        ...
        if activated_cat is None:
            ...
            num_of_sample = int(size_of_test_set_cbl / len(activated_cat))
        if num_of_sample > len(insts):
            print(
                f"Warning: Request {num_of_sample}, only {len(insts)} for category-{cat}!"
            )
...
def sampling_test_set_gbl(
    size_of_test_set_gbl, cat_inst: dict, cat_attr_inst: dict, used_inst: list, activated_cat:list=None
):
    ...
    for cat, insts in cat_inst.items():
        ...
        if activated_cat is None:
            num_of_sample = int(size_of_test_set_gbl / len(cat_inst))
        else:
            num_of_sample = int(size_of_test_set_gbl / len(activated_cat))
        if num_of_sample > len(insts):
            print(
                f"Warning: Request {num_of_sample}, only {len(insts)} for category-{cat}!"
            )
...
if __name__ == "__main__":
    ...
    cat_ratio, activated_cat = generate_long_tail_cat_probabilities(cat_inst)
    train_set = sampling_train_set(SIZE_OF_TRAIN_SET, cat_inst, used_inst, activated_cat)
    val_set = sampling_val_set(SIZE_OF_VAL_SET, cat_inst, used_inst, activated_cat)
    test_set = sampling_test_set(SIZE_OF_TEST_SET, cat_inst, used_inst, activated_cat)
    test_set_cbl = sampling_test_set_cbl(SIZE_OF_TEST_SET_CBL, cat_inst, used_inst, activated_cat)
    test_set_gbl, cat_attr_vec = sampling_test_set_gbl(
        SIZE_OF_TEST_SET_GBL, cat_inst, cat_attr_inst, used_inst, activated_cat
    )
    # print(train_set)
    # print(val_set)
    # print(test_set)
